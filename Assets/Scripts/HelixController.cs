...
public class HelixController : MonoBehaviour
{
    ...
    public void LoadStage(int stageNumber)
    {
        ...
        if (stage == null)
        {
            Debug.LogError("No stage" + stageNumber + "Found in allStage List. are all stages assined in the list");
            return;
        }
        ...
        //destroy the old level if there are any
        ...
        for (int i = 0; i < stage.levels.Count; i++)
        {
            ...
            int partsToDesable = 12 - stage.levels[i].partCount;
            List<GameObject> disabledParts = new List<GameObject>();

            while (disabledParts.Count < partsToDesable)
            {
